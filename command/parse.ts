import { PartialCommand, CommandParameter } from './command.ts';
    ...
    while (true) {
      ...
      for (let i = 0; i < argument.length; i++) {
        ...
        if (current == quote) {
          ...
          this.cursor++;
          let extra = argument.substring(i + 1);
          if (extra.length > 0) {
 ...
 * responsability is to fill the 'values' field
